...
*********************************************************************
*/
__global__ void fp16_linear_matmul_cuda(
  const half *input,
...
)
{
  ...
  if (col < n && row < m) {  
    ...
    for (int i = 0; i < k; i++) {
      ...
      if (weight == 1) {
        sum = __hadd(sum, input[row * k + i]);
      } else if (weight == 2) {
        sum = __hsub(sum, input[row * k + i]);
      } else if (weight == 3) {
        atomicExch(status, 1); // Set error flag, weights incorrectly packed
        return;
      }
    ...
    output[row * n + col] = __hadd(sum, bias[row]);
...
PYBIND11_MODULE(naive_linear_cuda, m) {
